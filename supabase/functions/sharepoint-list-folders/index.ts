...
serve(async (req) => {
  ...
  try {
    ...
    if (!driveId) {
      return new Response(
        JSON.stringify({ error: 'Drive ID is required' }),
    ...
    // Build URL based on whether we're listing root or a subfolder
    let url: string;
    if (folderId) {
      url = `https://graph.microsoft.com/v1.0/drives/${driveId}/items/${folderId}/children?$filter=folder ne null`;
    } else {
      url = `https://graph.microsoft.com/v1.0/drives/${driveId}/root/children?$filter=folder ne null`;
    }
    ...
    if (!foldersResponse.ok) {
      ...
      console.error('Folders fetch error:', errorText);
      return new Response(
        JSON.stringify({ error: 'Failed to fetch folders' }),
        { status: foldersResponse.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const foldersData = await foldersResponse.json();
    
    const folders = foldersData.value?.map((item: any) => ({
      id: item.id,
      name: item.name,
    ...
    console.error('Error in sharepoint-list-folders:', error);
