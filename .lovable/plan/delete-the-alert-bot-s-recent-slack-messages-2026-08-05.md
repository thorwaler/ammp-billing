# Delete the alert bot's recent Slack messages

Remove every message the alert bot posted yesterday (Aug 4, 2026) and so far today (Aug 5) in the two channels the automation routes to.
...
## Steps
1. Read the recent history of both channels through the Slack connector and select messages authored by the alert bot with a timestamp from Aug 4 00:00 CET through now.
