...
- Extend the `CachedCapabilities.unassignedOrgs` type with `coveredStandard`, `coveredEconf`, `uncovered` counts, an optional `excluded` count (only populated when `ammp_asset_group_id_not` is set), `uncoveredMW`, and a capped list (first 20) of `{ assetId, assetName, mw }` for the uncovered assets so the UI can name them.
